...
Tasks:
  ...
  11.7  Track B protocol subgroup analysis (XGBoost_B, DT_B, RF_B)
...
# Track B model → pred column (from phase9_test_predictions_b.parquet)
_B_PRED_COLS = {
    "XGBoost_B": "pred_xgboost_b",
    "DT_B":      "pred_dt_b",
    "RF_B":      "pred_rf_b",
}
# Display names matching Phase 9 metrics_test "Model" column
_B_DISPLAY_NAMES = {
    "XGBoost_B": "XGBoost B",
    "DT_B":      "DT B",
    "RF_B":      "RF B",
...
_TRACK_PROMPT = (
    ...
    "       Models: RF, XGBoost, LightGBM, DT, ANN, ElasticNet\n"
...
_B_MODEL_PROMPT = (
    "\n  Track B -- select models to include:\n"
    "  [1]  XGBoost_B only\n"
    "  [2]  DT_B only\n"
    "  [3]  RF_B only\n"
    "  [4]  All three (XGBoost_B, DT_B, RF_B)\n"
)


def _get_track_choice() -> set:
    while True:
        print(_TRACK_PROMPT)
        sys.stdout.flush()
...
def _get_b_model_choice() -> list:
    while True:
        print(_B_MODEL_PROMPT)
        sys.stdout.flush()
        raw = input("Enter choice: ").strip()
        if raw == "1":
            return ["XGBoost_B"]
...
def main():
    ...
    b_models_sel = []
    if run_b:
        b_models_sel = _get_b_model_choice()

    t0 = time.time()

    # Initialise outputs that may stay None if a track is skipped
    df = None
    metrics_df = None
    composition_df = None
    mw_df = pd.DataFrame()
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Tasks 11.0 – 11.6  (Track A)
    # ─────────────────────────────────────────────────────────────────────────
    if run_a:
        # ── Task 11.0 — Load data ────────────────────────────────────────────
        logger.info("=" * 60)
        logger.info("Task 11.0 -- Loading Track A data")
        logger.info("=" * 60)
        ...
        logger.info("Treatment/Control x HP type (%%):  \n%s", ct_hp.to_string())
        logger.info("Treatment/Control x PV (%%):        \n%s", ct_pv.to_string())
        logger.info("Treatment/Control x EV (%%):        \n%s", ct_ev.to_string())
        ...
        logger.info("Subgroup metrics computed: %d (subgroup x model) combinations", len(metrics_df))
        ...
        # ── Task 11.4 — Treatment effect analysis ────────────────────────────
        logger.info("=" * 60)
        ...
        if not mw_df.empty:
            ...
            for _, row in mw_df[mw_df["significant"]].iterrows():
                logger.info("  SIGNIFICANT: %s  %s vs %s  [%s]  Dmedian=%.2f  p_bonf=%.3e",
    ...
    if run_b:
        ...
        # Compute residual for each selected B model (only if pred column exists)
        b_pred_map = {}
        b_resid_map = {}
        for m in b_models_sel:
            pred_col = _B_PRED_COLS[m]
            if pred_col not in df_b.columns:
                logger.warning("%s not found in predictions file -- skipping %s", pred_col, m)
                continue
            resid_col = f"residual_{pred_col[5:]}"   # "pred_xgboost_b" -> "residual_xgboost_b"
            df_b[resid_col] = df_b["kWh_received_Total"] - df_b[pred_col]
            b_pred_map[m]  = pred_col
            b_resid_map[m] = resid_col
            logger.info("Computed %s for %s (n_valid=%d)", resid_col, m, df_b[resid_col].notna().sum())

        if not b_pred_map:
            logger.warning("No valid Track B prediction columns found -- skipping Task 11.7")
        else:
            # Primary model for figures (prefer XGBoost_B, else first available)
            primary_b = next((m for m in ["XGBoost_B", "DT_B", "RF_B"] if m in b_resid_map), None)
            primary_resid = b_resid_map[primary_b]
            primary_display = _B_DISPLAY_NAMES[primary_b]
            logger.info("Primary Track B model for figures: %s", primary_b)
            ...
            # run_subgroup_metrics for all selected B models
            b_display_pred = {_B_DISPLAY_NAMES[m]: col for m, col in b_pred_map.items()}
            track_b_metrics = run_subgroup_metrics(
                df_b, PROTOCOL_SG_MAP,
                pred_cols=b_display_pred,
                y_col="kWh_received_Total",
                min_n=10,
            )
            track_b_metrics.to_csv(TABLE_DIR / "phase11_track_b_subgroup_metrics.csv", index=False)
            logger.info("Track B metrics saved: %d rows (models: %s)",
                        len(track_b_metrics), list(b_display_pred.keys()))

            # Figures — use primary B model
            plot_track_b_bias_heatmap(
                track_b_metrics,
            ...
            if "HeatPump_Installation_CorrectlyPlanned" in df_b.columns:
                df_b["hp_sizing_label"] = df_b["HeatPump_Installation_CorrectlyPlanned"].map(
                    {1.0: "Correctly Planned", 0.0: "Incorrectly Planned",
                     True: "Correctly Planned", False: "Incorrectly Planned"}
                ).fillna("Unknown")
                plot_track_b_residual_boxplot(
            ...
            # HP issue flag Mann-Whitney for each selected B model
            from scipy.stats import mannwhitneyu as _mwu
            for m, resid_col in b_resid_map.items():
                for flag_col, flag_label in [
                    ...
                    if flag_col not in df_b.columns:
                        continue
                    yes = df_b[df_b[flag_col] == 1][resid_col].dropna()
                    no  = df_b[df_b[flag_col] == 0][resid_col].dropna()
                    if len(yes) >= 10 and len(no) >= 10:
                        stat, p = _mwu(yes, no, alternative="two-sided")
                        logger.info(
                            "[%s] HP issue [%s]: mean bias (flagged)=%.2f vs (not)=%.2f  p=%.3e",
                            _B_DISPLAY_NAMES[m], flag_label, yes.mean(), no.mean(), p,
                        )
    ...
    write_report(
        ...
        n_test_rows=len(df) if df is not None else 0,
        n_test_hh=df["Household_ID"].nunique() if df is not None else 0,
    )
